...
def run():
    ...
    min_word_size = 4
    max_word_size = 11
    # fixed matrix size (from count.bin)
    matrix_size = 256
    ...
    count = np.fromfile("count.bin", dtype="int32").reshape(matrix_size, matrix_size, matrix_size)
    s = count.sum(axis=2)
    st = np.tile(s.T, (matrix_size, 1, 1)).T
    ...
    for word_size in range(min_word_size, max_word_size):
        generated_words = 0
        while generated_words < nb_words:
            ...
            unicode_word = u''
            while not j == 10:
                unicode_code = choice(range(matrix_size), 1, p=markov_chain[i, j, :])[0]
